...
export const productService = {
  ...
  getNearbyProducts: (lat, lng, radius = 10) => api.get('/products/nearby', {
...
export const transactionService = {
  ...
  getUserTransactions: (status) => api.get('/transactions/my', { 
  ...
  completeTransaction: (id, rating, review) => api.put(`/transactions/${id}/complete`, {
    rating,
    review
  }),
  cancelTransaction: (id, reason) => api.put(`/transactions/${id}/cancel`, { reason }),
  requestTransaction: (productId, message) => api.post('/transactions/request', {
    productId,
    message
  }),
...
export const chatService = {
  ...
  createChat: (participantId, productId, initialMessage) => api.post('/chat', {
    participantId,
    productId,
    initialMessage
  }),
  getChatMessages: (chatId, page = 1, limit = 50) => api.get(`/chat/${chatId}/messages`, {
    params: { page, limit }
  }),
  sendMessage: (chatId, content, type = 'text') => api.post(`/chat/${chatId}/messages`, {
    content,
    type
  }),
  ...
  editMessage: (chatId, messageId, content) => api.put(`/chat/${chatId}/messages/${messageId}`, {
    content
  }),
};
...
export const notificationService = {
  ...
  updatePushSubscription: (subscription) => api.post('/notifications/push-subscription', subscription),
};
...
export const locationService = {
  ...
  getNearbyUsers: (lat, lng, radius = 10) => api.get('/location/nearby-users', {
    params: { lat, lng, radius }
  }),
...
export const adminService = {
  ...
  moderateProduct: (id, action, reason) => api.put(`/admin/products/${id}/moderate`, { action, reason }),
  deleteProduct: (id, reason) => api.delete(`/admin/products/${id}`, { data: { reason } }),
  
  // Reportes y moderación
  getReports: (params = {}) => api.get('/admin/reports', { params }),
  getReport: (id) => api.get(`/admin/reports/${id}`),
  resolveReport: (id, resolution, notes) => api.put(`/admin/reports/${id}/resolve`, { resolution, notes }),
  ...
  // Analytics avanzados
  getAnalytics: (type, period = '30d') => api.get('/admin/analytics', { 
    params: { type, period } 
  }),
  exportData: (type, format = 'csv') => api.get('/admin/export', {
    params: { type, format },
    responseType: 'blob'
  }),
};
